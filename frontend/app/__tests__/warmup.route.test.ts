/**
 * Unit tests for app/warmup/route.ts — the warmup passthrough proxy (H3).
 *
 * Verifies:
...
describe("POST /warmup route handler (H3)", () => {
  beforeEach(() => {
  ...
  it("proxies to BACKEND_ORIGIN/warmup and returns the upstream response", async () => {
    vi.stubGlobal(
      "fetch",
    ...
    const { POST } = await import("@/app/warmup/route");
