...
- **Redis fail-open 須搭 socket 逾時**（Codex 終審 P2#2）：對「例外」fail-open 對「無限 hang」無效，故 production redis client 設 `socket_timeout`/`socket_connect_timeout`（`redis_socket_timeout_seconds`，預設 0.5s），把 stall 轉成例外讓 cache/ratelimit 真正 fail-open。
- **真實 S3 接線時的快取規則**（Codex 終審 P2#1，待 boto3/S3 接線）：`sources.image_url` 為 sign_url 產物；真實 presigned URL 會過期而 cache TTL 達 14 天，命中可能回到過期 URL。接線時 **MUST** 改存穩定 `page_image_uri` 並於命中時重簽（或令 cache TTL ≤ presign 壽命）。目前 mock 不過期、真實模式 sign_url 尚 `NotImplementedError`，故暫無影響。
...
2. **假名化 trace id 化解 §6.5×D-M**：LangFuse 只收 **HMAC-SHA256(key=高熵 salt, msg=user_id)** 假名（128 bits；§6.5「可追蹤」），**缺 salt 則不啟用 LangFuse**（防低熵 ID 字典反查）；trace metadata 採 **key+value allowlist**（value 限 primitive 或 ≤16 字短 enum）；**score name 固定 allowlist、comment 不外送**；**MUST NOT** 收原始 user_id/學號/query/檢索內容（D-M）。
3. **Sentry 結構 allowlist 脫敏（D-M）**：`send_default_pii=False` + `max_request_body_size="never"` + `include_local_variables=False` + `before_send` **只保留核准頂層欄位 + 例外 type/stacktrace 結構（frame 只留 module/function/lineno/in_app）+ 核准 contexts 型別的核准子欄位（device 整個丟棄）**，其餘（user/tags/extra/breadcrumbs/request/message/logentry/未知）整塊丟棄；非 dict/出錯→回 None 丟棄 event。保留的頂層/結構欄位由 Sentry 以伺服器端來源填充（hostname/route/套件版/code 位置），本架構不將使用者內容寫入這些欄位（**程式碼終審以無洩漏斷言測試守門**）。
...
### 後果
- 設 LangFuse/Sentry 憑證才真正送出；未設則靜默 no-op（log 仍在）。Phase 9 mock-first→零外部流量、零費用。
...
6. **API proxy**：前端 **runtime Route Handler**（`app/{chat,feedback,warmup}/route.ts`，runtime 讀 `BACKEND_ORIGIN`、`/chat` 串流 `upstream.body`、上游不可達回 502、不轉發 cookie）代理至後端（compose `backend:8000`），同源、免 CORS。**取代 next.config rewrites**（rewrites 會在 build 期 bake `BACKEND_ORIGIN`→Docker 部署斷線）。
...
## DL-028: 評估框架 v1＝mock-first；真實 §7.3 hard gate / 持久化 / 自動促進 屬 live/ops，gated

- **狀態**：APPROVED（委派，2026-06-14；使用者於 Phase 11 規劃核可 mock-first + framework-only）　**提案者**：main Claude（Phase 11）　**日期**：2026-06-14　**裁決者**：專案負責人
- **影響檔案**：`eval/`（runner/gate/recall/retention/kappa/regression）、`eval/eval_thresholds.yaml`、`tests/golden_qa.jsonl`、`.github/CODEOWNERS`、`docs/golden-contribution-guide.md`

### 背景
§7.3 每 PR 真實 RAGAS hard gate + §7.2 ≥110 教師題 + §7.6 ≥90d 持久化 + §7.4 錯誤自動促進——皆卡在「教師題庫 + 付費 eval key + 持久化後端 + 排程」（與 D1/D2 規劃決定一致）。Phase 11 需在零費用、零外部呼叫下交付可測框架，並誠實記錄與 §7 完整 live 需求的偏離（比照 DL-026 告警「邏輯先行、operational 延後」）。

### 提案（framework-only，ops gated）
1. **框架交付範圍**：runner/gate（NaN-safe）/recall-by-class/retention（UTC robust，≥90d）/kappa（pe≈1→nan）/regression（in-memory 驗證）/Streamlit 抽檢/`eval_thresholds.yaml`（active/pending 分層）/golden schema+模板+就緒檢查/CI offline+ragas 假件。
2. **offline 決定性 gate 每 PR 跑**：schema/recall-by-class/gate 邏輯/kappa/regression/retention 單元 + ragas wiring 假件（決定性無-LLM 指標，零費用、零外呼叫）。
3. **真實 RAGAS 品質 gate**：`workflow_dispatch`（需 `EVAL_OPENAI_KEY` + golden ready）；**未滿 ≥110 教師題前，readiness 為 warning 不阻 merge**（DL-028 誠實，教師填滿後由人工升 hard gate）。
4. **active/pending 門檻分層**（`eval_thresholds.yaml`）：`ragas.active`（faithfulness/answer_relevancy/context_precision/out_of_scope_correctness）本 phase gate 檢查；`ragas.pending`（context_recall）待教師 reference 答案後啟用，**gate 不檢查**，僅報告。
5. **OOS 正確性**：由生成層 `out_of_scope_correctness` 指標量，**不計 retrieval recall**（釐清 DL-013）；recall-by-class 涵蓋 4 個檢索類別（text_only/figure_id/cross_page/clinical_correlation）。
6. **RAGAS 版本**：`ragas==0.4.3`（精確 pin）；offline 決定性核心用 `NonLLMContextRecall`/`RougeScore`/純 Python OOS（llm=None 安全，零 fallback）；LLM 指標（Faithfulness/ResponseRelevancy/LLMContextPrecision）僅在 `--real`/`workflow_dispatch` 跑，CI 只做「不打網路、回 dict」薄 wiring 測試（注入假件、patch OpenAI、清 key）。
7. **§7.6 持久化後端 / §7.4 自動促進 / 排程 regression job**：屬 ops，刻意 gated（先問，後續）。
8. **不新增 runtime 套件**：`pyyaml>=6` 已在 eval base deps（gate 讀 yaml）；ragas/langchain-openai/datasets 為 `eval[ragas]` extra（CI offline job 不裝）。

### 後果
- CI 每 PR 跑 offline + ragas 假件（零費用）；真實 RAGAS 分數待教師題 + eval key；**Phase 11 不宣稱 §7 全 live**。
- §7.2 ≥110 教師題：readiness 為 warning 不阻 merge；真實 gate 升 hard 須人工操作（CODEOWNERS）。
- §7.3 live gate：待 `EVAL_OPENAI_KEY` + golden ready，走 `workflow_dispatch`。
- §7.6 持久化保留：`retention.py` 框架已在，持久化後端（S3/NFS）屬 ops 後續。
- 門檻不得調降；`eval_thresholds.yaml`/`tests/golden_qa.jsonl` 走 `.github/CODEOWNERS` 人工審。
- 回寫 §7.2/§7.3 標注「live gate 待教師題庫 + eval key」。
