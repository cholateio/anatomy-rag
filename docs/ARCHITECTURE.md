...
T+200ms [Encoder] POST /encode_query → {tokens_bin[~20], pooled_bin}
                  (校內 5060 Ti ~50ms / Modal fallback ~200ms)
T+230ms [DB]    Stage A: HNSW on pooled_bin + metadata 過濾 → 50 候選 page_ids
...
| Encoder（校內主 / Modal 備） | < 100ms / < 300ms | `encoder.latency` |
| Stage A | < 30ms | `retrieval.stage_a.latency` |
| Stage B（50 候選） | < 200ms | `retrieval.stage_b.latency` |
...
| ColPali query 編碼忘了 mean-pool | Stage A 沒有 pooled_bin 可比 | encoder 服務直接回兩個欄位 |
...
每頁額外計算：**mean-pooled 向量**（128-dim → 二值化）存 `pages.pooled_bin`（Stage A 用）；**metadata JSONB** 存 `pages.metadata`。
...
**契約**：`binarize(vec_f32: Tensor[128]) -> bytes(16)`，sign-based（`>0` 取 1，否則 0；bit i 對應 `out[i//8]` 的第 `7-(i%8)` 位）。pooled 向量：`binarize(patch_embs.mean(dim=0))`；patch 向量：逐一 `binarize`。

**規則**：
- **MUST** patch 向量與 pooled 向量都二值化後寫入資料庫
- **MUST** 查詢時使用**相同**的二值化函式（離線端與 query 端**共用同一份** `shared/binary.py`，兩端 import；不一致會直接讓檢索精度崩壞）
- **MUST**（v1 預設）資料庫只存 bit 版本，不存 float32。**MAY**（升級路徑，DL-003）當 RAGAS `context_precision < 0.85` 且確認 binary 量化為瓶頸時，另存一份**更高精度的 rescore 表示**供 Stage B 精排——**優先 INT8**（相對 float32 省 4×、相對 binary 品質明顯回升），float32 為次選；啟用須經 RAGAS 與儲存評估並走 `decisions.md`。
...
CREATE TABLE pages (                          -- 頁面層：Stage A 與 LLM payload 來源
    ...
    pooled_bin      BIT(128) NOT NULL,         -- mean-pooled patch 向量再 binarize
    text_tsv        TSVECTOR
...
```sql
-- Stage A 粗排：HNSW on Hamming
CREATE INDEX pages_pooled_hnsw ON pages USING hnsw (pooled_bin bit_hamming_ops)
...
encoder service 回傳 `{"tokens_bin": [bytes_16, ...N], "pooled_bin": bytes_16}`：`pooled_bin` 給 Stage A、`tokens_bin` 給 Stage B。完整部署見 [§5.1](#51-colpali-query-encoder-部署)。
...
ORDER BY pooled_bin <~> :query_pooled_bin
LIMIT :top_k;
```
...
token_max_per_page AS (
    SELECT pp.page_id, qt.token_idx,
           MAX(128 - (pp.patch_bin <~> qt.q_bin::bit(128)))::float AS sim
...
**規則**：
- **MUST（DL-013）**：中英混合 query 時，BM25 餵 **DL-008 翻譯後的英文 query**（tsvector 為英文，原始中文 query 會空轉），原始混語 query 保留給生成
...
POST /encode_query
Request:  {"q": "肱二頭肌的起止點"}
Response: {"tokens_bin": ["base64...", ...], "pooled_bin": "base64...", "model": "colpali-v1.3-hf"}
```
...
1. 限流兜底 → 2. 語意快取查詢（**本地 embedding**，命中直接 yield `sources`+`delta`+`done` 並 `create_task` log，return）
3. 編碼 query → 4. 兩階段檢索 + RRF（`retrieve(...) -> list[RetrievalResult]`）；**檢索＋圖片 fetch 完成即歸還 DB 連線，不得跨 LLM 串流持有（DL-012）**
...
| HNSW（每頁 1 pooled 向量） | 數 MB（百萬頁仍輕） | 非瓶頸 |
